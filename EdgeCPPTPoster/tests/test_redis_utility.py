...
with patch.dict("os.environ", {'redis_secret_name': 'test_secret_name', 'region': 'us-east-1',
                               "LoggingLevel": "debug"}):
...
class TestRedisUtility(TestCase):
    ...
    @patch("utilities.redis_utility.read_from_the_edge_database")
    ...
    def test_getSetRedisValue_whenGetSetRedisValueIsCalledAndConnectionObjectAvailableWithoutCache_thenDBCalled(
        ...
        mock_read_from_the_edge_database.return_value = [{"test": "test"}]

        result = get_set_redis_value("test_key", "test_query", 3600)

        self.assertEqual(result[0]["test"], "test")
        mock_read_from_the_edge_database.assert_called_with("test_query")
