...
aws cloudformation deploy --stack-name da-edge-pre-reqs-j1939-dest --template-file ToolsAcct/pre-reqs.yaml --parameter-overrides DevAccount=$DevAccount StageAccount=$StageAccount ProductionAccount=$ProdAccount --profile $ToolsAccountProfile
...
aws cloudformation deploy --stack-name da-edge-pipeline-j1939 --template-file ToolsAcct/code-pipeline.yaml --parameter-overrides DevAccount=$DevAccount StageAccount=$StageAccount ProductionAccount=$ProdAccount CMKARN=$CMKArn CMKARNPROD=$CMKArnProd CMKARNIndia=$CMKArnIndia S3Bucket=$S3Bucket S3BucketProd=$S3BucketProd S3BucketIndia=$S3BucketIndia --capabilities CAPABILITY_NAMED_IAM --profile $ToolsAccountProfile
